...
function clearCart() {
    ...
    if (confirm('Bạn có chắc muốn xóa toàn bộ giỏ hàng?')) {
        ...
        document.querySelectorAll('.qty-display').forEach(element => {
    ...
    if (navigator.clipboard) {
        navigator.clipboard.writeText(message).then(() => {
            ...
            setTimeout(() => {
                ...
                copyBtn.textContent = '📋 Copy Đơn Hàng';
            }, 2000);
        }).catch(() => {
...
function orderViaZalo() {
    ...
    // Tự động copy message vào clipboard
    if (navigator.clipboard) {
        navigator.clipboard.writeText(message).then(() => {
            // Hiển thị hướng dẫn
            showToast('✅ Đã copy! Mở Zalo và paste tin nhắn');
            ...
            // Fallback nếu copy không thành công
            alert('📋 Đã copy đơn hàng!\n\nVui lòng paste vào Zalo để gửi.');
            window.open(zaloLink, '_blank');
        });
    } else {
        // Fallback cho trình duyệt cũ
        ...
        try {
            ...
            document.body.removeChild(textArea);
            alert('📋 Đơn hàng:\n\n' + message + '\n\nVui lòng copy và paste vào Zalo!');
            window.open(zaloLink, '_blank');
        }
    }
